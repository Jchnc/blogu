...
export default function FeaturedArticle({ article }: { article?: Article }) {
  ...
  const createdAt = article?.createdAt || "xxxx-xx-xx";
  ...
  return (
    <article className="flex flex-col my-10">
      <Link href="">
        <picture>
          <img src={image} alt={title} loading="lazy" className="rounded-2xl w-full object-cover" />
        </picture>
        <p className="text-gray-400 my-4 text-sm flex items-center gap-2">
          <Person size={14} /> {String(author)}
          <Calendar size={14} /> {String(createdAt)}
        </p>
